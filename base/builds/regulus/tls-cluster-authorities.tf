...
resource "tls_private_key" "kubernetes-ca-key" {
  algorithm = "RSA"
  rsa_bits = 4096
}

resource "tls_cert_request" "kubernetes-ca" {
  private_key_pem = tls_private_key.kubernetes-ca-key.private_key_pem
  dns_names = [ "kubernetes" ]
...
resource "tls_cert_request" "front-proxy-ca" {
  private_key_pem = tls_private_key.front-proxy-key.private_key_pem
